...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    setupUploadZone('decoyDropZone', 'decoyImageInput', 'decoyPlaceholder', 'decoyPreview', 'decoyInfo', (data) => {
        decoyImageData = data;
        if (document.getElementById('stegoAutoFitCheck').checked && originalSecretImageSrc) {
    ...
    setupUploadZone('secretDropZone', 'secretImageInput', 'secretPlaceholder', 'secretPreview', 'secretInfo', (data) => {
        ...
        if (document.getElementById('stegoAutoFitCheck').checked) {
            triggerAutoFitIfEnabled();
        } else {
            updateStegoUI();
        }
    ...
    // Compression / Scaling Logic
    function scaleSecretImage(scaleValue) {
        if (!originalSecretImageSrc) return Promise.resolve();
        return new Promise((resolve) => {
            const img = new Image();
            img.onload = () => {
                const canvas = document.createElement('canvas');
                canvas.width = Math.max(1, Math.floor(img.width * scaleValue));
                canvas.height = Math.max(1, Math.floor(img.height * scaleValue));
                const ctx = canvas.getContext('2d');
                ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
                
                // Lower JPEG quality slightly as scale goes down to compress more
                const quality = scaleValue < 1.0 ? Math.max(0.1, scaleValue) : 0.92;
                const newDataURL = canvas.toDataURL('image/jpeg', quality);
                
                // Have to extract image data again
                const imgParsed = new Image();
                imgParsed.onload = () => {
                    ctx.clearRect(0,0, canvas.width, canvas.height);
                    ctx.drawImage(imgParsed, 0, 0);
                    const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
                    secretImageData = {
                        width: canvas.width,
                    ...
                    document.getElementById('secretInfo').textContent = `${canvas.width}×${canvas.height}px (Scaled ${(scaleValue*100).toFixed(0)}%)`;
                    updateStegoUI();
                    resolve();
                };
    ...
    if(stegoSlider) {
        stegoSlider.addEventListener('input', async (e) => {
            if (stegoAutoFit.checked) {
                stegoAutoFit.checked = false; // Disable auto if user manually slides
            }
    ...
    async function triggerAutoFitIfEnabled() {
        if (!stegoAutoFit.checked || !decoyImageData || !originalSecretImageSrc) return;
        ...
        // Binary search for the highest scale that fits
        let low = 0.05;
        let high = 1.0;
        let bestScale = 0.05;
        ...
        // Check if 1.0 already fits perfectly
    ...
    function writeBits(decoyData, bitOffset, value, numBits) {
        // Use unsigned right shift (>>>) to correctly handle values > 2^31
        const uValue = value >>> 0;
        for (let i = numBits - 1; i >= 0; i--) {
            const bit = (uValue >>> i) & 1;
    ...
    function readBits(stegoData, bitOffset, numBits) {
        // Accumulate as a double-precision float to avoid 32-bit signed overflow
        let value = 0;
        for (let i = numBits - 1; i >= 0; i--) {
            const pixelIndex = Math.floor(bitOffset / BITS_PER_DECOY_PIXEL);
            const channelInPixel = Math.floor((bitOffset % BITS_PER_DECOY_PIXEL) / BITS_PER_CHANNEL);
    ...
    let currentMailComposeAttachment = null;
    ...
    // Populate mail recipient select
    function refreshMailRecipients() {
        const sel = document.getElementById('mailRecipientSelect');
        if (!sel) return;
        sel.innerHTML = '';
        const contacts = getContacts();
        const myKeys = getRSAKeys();
    ...
    function renderMailList() {
        ...
        const countBadge = document.getElementById('mailInboxCount');
        if (!list) return;

        const items = mb[activeMailFolder] || [];

        // Update inbox count
        const unreadCount = (mb.inbox || []).filter(m => !m.read).length;
        if (countBadge) countBadge.textContent = unreadCount;

        if (items.length === 0) {
            list.innerHTML = `<div class="mail-empty"><span>📭</span><p>No messages in ${activeMailFolder}</p></div>`;
            return;
        }
        ...
        items.slice().reverse().forEach(mail => {
            const div = document.createElement('div');
            div.className = `mail-list-item${mail.id === activeMailId ? ' active' : ''}${!mail.read && activeMailFolder === 'inbox' ? ' unread' : ''}`;
            const dateStr = mail.timestamp ? new Date(mail.timestamp).toLocaleDateString() : '';
            const fromLabel = activeMailFolder === 'sent' ? `To: ${mail.to || '???'}` : (mail.from || 'Unknown');
            const preview = mail.decrypted ? (mail.body || '').substring(0, 60) : '🔒 Encrypted';
            
            div.innerHTML = `
                <div class="mail-item-top">
                    <span class="mail-item-from">${escapeHTML(fromLabel)}</span>
                    <span class="mail-item-date">${dateStr}</span>
                </div>
                <div class="mail-item-subject">${escapeHTML(mail.subject || '(No Subject)')}</div>
    ...
    function openMail(mailId) {
        ...
        document.getElementById('mailComposeView').style.display = 'none';
        document.getElementById('mailReadView').style.display = 'flex';

        document.getElementById('mailReadSubject').textContent = mail.subject || '(No Subject)';
        document.getElementById('mailReadFrom').textContent = activeMailFolder === 'sent' ? `To: ${mail.to}` : `From: ${mail.from || 'Unknown'}`;
        document.getElementById('mailReadDate').textContent = mail.timestamp ? new Date(mail.timestamp).toLocaleString() : '';

        const bodyEl = document.getElementById('mailReadBody');
        const imgEl = document.getElementById('mailReadImg');
        const decryptBtn = document.getElementById('mailReadDecrypt');

        if (mail.decrypted) {
            bodyEl.innerHTML = `<div style="white-space: pre-wrap;">${escapeHTML(mail.body || '')}</div>`;
            decryptBtn.style.display = 'none';
            if (mail.image) {
                imgEl.src = mail.image;
                imgEl.style.display = 'block';
            } else {
                imgEl.style.display = 'none';
            }
        } else {
            bodyEl.innerHTML = `<div class="mail-encrypted-placeholder"><span>🔒</span><p>This message is encrypted. Click Decrypt to reveal.</p></div>`;
    ...
    // Decrypt mail
    document.getElementById('mailReadDecrypt')?.addEventListener('click', () => {
        if (!activeMailId) return;
        const mb = getMailbox();
        const items = mb[activeMailFolder] || [];
        const mail = items.find(m => m.id === activeMailId);
        if (!mail || mail.decrypted || !mail.encryptedPayload) return;
        ...
        try {
            ...
            if (d.finish()) {
                const decryptedStr = d.output.toString('utf8');
                let parsedBody = decryptedStr;
                let parsedImage = null;

                try {
                    const obj = JSON.parse(decryptedStr);
                    if (obj && typeof obj.subject !== 'undefined') {
                        parsedBody = obj.body || '';
                        parsedImage = obj.image || null;
                        mail.subject = obj.subject || mail.subject;
                    }
                } catch (e) {}

                mail.body = parsedBody;
                if (parsedImage) mail.image = parsedImage;
                mail.decrypted = true;
                saveMailbox(mb);
                openMail(activeMailId);
                renderMailList();
                showChatToast('Mail decrypted! 🔓');
                playSound('receive');
            } else {
                showMessage('Integrity check failed!', 'error');
            }
        } catch (e) {
            console.error('Mail decrypt error:', e);
            showMessage('Decryption failed! Not for you? 🤔', 'error');
        }
    });
    ...
    document.getElementById('mailComposeBtn')?.addEventListener('click', () => {
        ...
        document.getElementById('mailEmptyState').style.display = 'none';
        document.getElementById('mailReadView').style.display = 'none';
        document.getElementById('mailComposeView').style.display = 'block';
        document.getElementById('mailSubject').value = '';
        document.getElementById('mailBody').value = '';
        currentMailComposeAttachment = null;
        const prevEl = document.getElementById('mailComposeAttachPreview');
    ...
    document.getElementById('mailComposeAttachBtn')?.addEventListener('click', () => {
        document.getElementById('mailComposeAttachInput')?.click();
    });

    document.getElementById('mailComposeAttachInput')?.addEventListener('change', (e) => {
        const file = e.target.files[0];
        if (!file) return;
        if (file.size > 5 * 1024 * 1024) { showMessage('Image too large (max 5MB)', 'error'); e.target.value = ''; return; }
        const reader = new FileReader();
        reader.onload = (ev) => {
            currentMailComposeAttachment = ev.target.result;
            const img = document.getElementById('mailComposeAttachImg');
            const prev = document.getElementById('mailComposeAttachPreview');
            if (img) img.src = currentMailComposeAttachment;
            if (prev) prev.style.display = 'flex';
        };
        reader.readAsDataURL(file);
    });

    document.getElementById('mailComposeAttachClear')?.addEventListener('click', () => {
        currentMailComposeAttachment = null;
        const img = document.getElementById('mailComposeAttachImg');
        const prev = document.getElementById('mailComposeAttachPreview');
        const input = document.getElementById('mailComposeAttachInput');
        if (img) img.src = '';
    ...
    document.getElementById('mailSendBtn')?.addEventListener('click', async () => {
        const recipientName = document.getElementById('mailRecipientSelect')?.value;
        const subject = document.getElementById('mailSubject')?.value?.trim() || '(No Subject)';
        const body = document.getElementById('mailBody')?.value?.trim() || '';

        if (!recipientName) return showMessage('Select a recipient!', 'error');
        if (!body && !currentMailComposeAttachment) return showMessage('Write something or attach an image!', 'error');

        const myKeys = getRSAKeys();
        if (!myKeys) return showMessage('No identity!', 'error');

        const contacts = getContacts();
        let pubKeyPem;
        if (recipientName === 'Note to Self') {
            pubKeyPem = myKeys.publicKey;
        } else {
            const c = contacts.find(x => x.name === recipientName);
            if (c) pubKeyPem = c.publicKey || c.key;
        }
        if (!pubKeyPem) return showMessage('Contact key not found!', 'error');

        try {
            const payloadObj = { subject, body, image: currentMailComposeAttachment };
            ...
            mb.sent.push({
                ...
                subject,
                body,
                image: currentMailComposeAttachment,
                encrypted: encryptedStr,
                timestamp: new Date().toISOString(),
                decrypted: true,
                read: true
            ...
            if (sentViaP2P) {
                showChatToast('Mail sent via P2P! ⚡');
            } else if (recipientName === 'Note to Self') {
                // Also save to inbox for self
                mb.inbox.push({
                    id: generateMailId(),
                    from: 'Me',
                    subject,
                ...
                saveMailbox(mb);
                renderMailList();
                showChatToast('Note saved to mailbox! 📝');
            } else {
                showChatToast('Friend offline — mail queued for P2P delivery 📬');
            }
    ...
    // Save draft
    document.getElementById('mailSaveDraftBtn')?.addEventListener('click', () => {
        const recipientName = document.getElementById('mailRecipientSelect')?.value || '';
        const subject = document.getElementById('mailSubject')?.value?.trim() || '(No Subject)';
        const body = document.getElementById('mailBody')?.value?.trim() || '';
        ...
        document.getElementById('mailEmptyState').style.display = 'flex';
        renderMailList();
        showChatToast('Draft saved! 💾');
    });

    // Delete mail
    document.getElementById('mailReadDelete')?.addEventListener('click', () => {
        if (!activeMailId) return;
        if (!confirm('Delete this message?')) return;

        const mb = getMailbox();
        const items = mb[activeMailFolder] || [];
        mb[activeMailFolder] = items.filter(m => m.id !== activeMailId);
        saveMailbox(mb);
        ...
        document.getElementById('mailEmptyState').style.display = 'flex';
        renderMailList();
        showChatToast('Deleted! 🗑️');
    });

    // Reply
    document.getElementById('mailReadReply')?.addEventListener('click', () => {
        if (!activeMailId) return;
        const mb = getMailbox();
        const items = mb[activeMailFolder] || [];
        const mail = items.find(m => m.id === activeMailId);
        ...
        if (sel && mail.from) {
            for (let i = 0; i < sel.options.length; i++) {
                if (sel.options[i].value === mail.from || sel.options[i].textContent === mail.from) {
                    sel.selectedIndex = i;
                    break;
                }
            }
    ...
    // Back from read
    document.getElementById('mailReadBack')?.addEventListener('click', () => {
        activeMailId = null;
        document.getElementById('mailReadView').style.display = 'none';
        document.getElementById('mailEmptyState').style.display = 'flex';
        renderMailList();
    ...
    // Folder switching
    document.querySelectorAll('.mail-folder-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            document.querySelectorAll('.mail-folder-btn').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
    ...
    // Handle receiving mail via P2P - extend the existing P2P handler
    const originalHandleP2P = window._handleP2PData;
    window._handleP2PData = function(data) {
        if (data.type === 'encrypted-mail') {
            const contacts = getContacts();
            let senderName = 'Unknown';
            if (data.senderKey) {
                const c = contacts.find(x => (x.publicKey || x.key) === data.senderKey);
                if (c) senderName = c.name;
            }
            ...
            mb.inbox.push({
                id: generateMailId(),
                from: senderName,
                subject: data.subject || '(Encrypted)',
                encryptedPayload: data.payload,
                timestamp: data.timestamp || new Date().toISOString(),
                decrypted: false,
                read: false
            ...
            renderMailList();
            playSound('receive');
            showChatToast(`📬 New mail from ${senderName}!`);
            return;
        }
        // Fall through to original handler
        if (originalHandleP2P) originalHandleP2P(data);
    };

    // Refresh mail on tab switch
    const mailTabObserver = new MutationObserver(() => {
        const mailTab = document.getElementById('mail-tab');
        if (mailTab && mailTab.classList.contains('active')) {
            refreshMailRecipients();
            renderMailList();
        }
    });
    const mailTab = document.getElementById('mail-tab');
    if (mailTab) {
        mailTabObserver.observe(mailTab, { attributes: true, attributeFilter: ['class'] });
    }
