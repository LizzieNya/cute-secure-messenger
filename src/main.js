...
async function encryptWithPerfectForwardSecrecy(
  ...
  try {
    ...
    const senderProof = keyPair ? Buffer.from(keyPair.exportKey("pkcs1-public-pem")).toString("base64") : "PFS-v2.0";
...
async function decryptWithPerfectForwardSecrecy(encryptedEnvelopeJson) {
  try {
    ...
    if (signature && signature !== "signature-placeholder") {
      if (!verifySignature(JSON.stringify(envelope), signature, signedEnvelope.senderProof)) {
...
function verifySignature(message, signature, senderProof) {
  try {
    if (!senderProof || senderProof.startsWith("PFS-")) return true; // Legacy fallback

    const publicKeyPem = Buffer.from(senderProof, "base64").toString("utf8");
    const testKey = new NodeRSA();
    testKey.importKey(publicKeyPem, "pkcs1-public-pem");

    return testKey.verify(message, signature, "utf8", "base64");
  } catch (error) {
  ...
  try {
    ...
    for (const [name, publicKey] of Object.entries(recipientPublicKeys)) {
      try {
        ...
        const keyPair = loadOrCreateMyKeyPair();
        const senderProof = keyPair ? Buffer.from(keyPair.exportKey("pkcs1-public-pem")).toString("base64") : "PFS-MULTI-v2.0";
